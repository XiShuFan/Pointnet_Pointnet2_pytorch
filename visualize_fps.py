...
def output_ply(cell_true_labels, cells, points, file_name):
    """
    把面片输出到ply文件中
    """
    header = f"ply\n" \
...
def fps_once(config, xyz, points, select_index, info, target_path):
    xyz_new, points_new, _, fps_index = pointnet2_utils.sample_and_group(npoint=config['npoint'], radius=config['radius'],
                                                                         nsample=config['nsample'], xyz=xyz, points=points, returnfps=True)
    ...
    # 打算用xyz数据画一个点云图，然后再输出口扫的可视化图
    select_index = select_index[fps_index.squeeze(0).cpu().numpy()]
    output_ply(select_index, info['faces'], info['vertices'],
